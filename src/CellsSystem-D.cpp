...
// 
// 	v.clear();
#if 0 // this is the working configuration, even if it is slow
  for(k=0; k<ncells; k++)
  {
...
// #pragma omp parallel for
  ...
  printf("xmin: %f, xmax: %f, ymin: %f, ymax: %f, zmin: %f, zmax: %f", min_x, max_x, min_y, max_y, min_z, max_z);
	/** Construct the locking data-structure, using the bounding-box of the points
   * if no cell is present, the definition of the max's and min's not corrent, 
   * since the loop is not executed. In this case I overwritte the values.
   */
...
#endif
#ifndef NDEBUG
  std::cout << "triangulation ready" << std::endl;
#endif

  /**  NEIGHBORS
   * next we find the list of connections and we estimate contact areas
   * to make this part in parallel I:
   * 
   * 1) retrieve the information about the triangulation in serial from CGAL
   *    and store them in rr_of_vn_pointers 
   *    MAYBE this explodes!!!
   *    for the examples this war quite fast
   * 2) executed the all computations in the multi threaded environment
   */
	//this vector stores all the neigbourhood information
	
  #if 1
  Delaunay::Finite_vertices_iterator vit;
  //loop to retrieve information, NOTE serial
	for (vit = DelTri.finite_vertices_begin(); vit != DelTri.finite_vertices_end(); ++vit)
  {
    k = vit->info();
...
// sia equamente suddivisa tra vicini e ambiente
  ...
  }//end for (Finite_vertices_iterator vit
...
#endif
#endif //#ifdef CGAL_LINKED_WITH_TBB
}

void CellsSystem::Geometry_serial()
{
//   typedef CGAL::Exact_predicates_inexact_constructions_kernel     	K;
// 
//   typedef CGAL::Triangulation_vertex_base_with_info_3<unsigned, K> 	Vb;
//   typedef CGAL::Fixed_alpha_shape_vertex_base_3<K, Vb>      			AsVb;
//   typedef CGAL::Fixed_alpha_shape_cell_base_3<K>        				Fb;
//   typedef CGAL::Triangulation_data_structure_3<AsVb,Fb,CGAL::Fast_location> Tds;
//   typedef CGAL::Delaunay_triangulation_3<K,Tds> Delaunay;
//   typedef CGAL::Fixed_alpha_shape_3<Delaunay>Fixed_alpha_shape_3;
// 
//   // ****************
